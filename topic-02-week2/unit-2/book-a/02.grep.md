...
```
NOTE: IT IS CASE SENSITIVE SO IT IS WRITTEN AS
...
(Otherwise you might need to use the backslash to escape characters if using basic `grep`)
...
```bash
$ grep -E 'm[ao]*' employee.txt
```
And what about:

```bash
$ grep -E 'ma[ao]*' employee.txt
```

## ^ to search the beginning of the line

```bash
$ grep -E '^.00' employee.txt
```
...
```bash
$ grep -E '$' employee.txt # what does this return
```

and now try:

```bash
$ grep -E '\$' employee.txt # spot the difference
```
