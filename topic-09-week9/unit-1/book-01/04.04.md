...
+ Click on the *Run* button and check out the response. You'll get a status code and data, if any. What's the value of the status code and what does is mean?
...
+ Replace the code with the following ,and modify the URLs IP address to refer to your Raspberry Pi.
...
# Define the URL of the REST API endpoint that controls the LEDs
resourceURL = "http://172.24.10.250:5000/api/leds"

# Set GPIO pin 0 as an input (for the motion sensor)
pinMode(0, IN)

# Create an HTTP client instance
http = RealHTTPClient()

# Event handler function for the sensor input on digital port 0
def handleSensorEvent():
	print("Motion Detected")
	sensorState = digitalRead(0)  # Read the current state of the motion sensor
	if sensorState == LOW:
		# If sensor state is LOW, send a PUT request to turn LEDs off
		http.put(resourceURL, '{"state":"off"}')
	else:
		# If sensor state is HIGH, send a PUT request to turn LEDs on
		http.put(resourceURL, '{"state":"on"}')

# Callback function to handle HTTP responses
def onHTTPDone(status, data):
    print("status: " + str(status))  # Print the HTTP status code
    print("data: " + data)           # Print the response data from the server

def main():
    # Register the event handler for changes on digital port 0
    add_event_detect(0, handleSensorEvent)
    
    # Register the callback to be triggered when HTTP requests complete
    http.onDone(onHTTPDone)

    # Keep the program running indefinitely so it can respond to events
    while True:
        sleep(1)

# Entry point: run main() if this file is executed directly
