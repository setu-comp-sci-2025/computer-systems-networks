...
+ Turn on the LED array (light) using a  ``HTTP POST`` request to ``/api/leds`` with a body  ``{"state":"on"}`` as follows:

There are slight differences in how CURL works depending on your OS. 

**Windows (command prompt (CMD))**

```bash
curl -X PUT http://hdiprpi:5000/api/leds -H "Content-Type: application/json" -d "{\"state\":\"on\"}"
```

+ Turn it off using ``{"state":"off"}``

```bash
curl -X PUT http://hdiprpi:5000/api/leds -H "Content-Type: application/json" -d "{\"state\":\"off\"}"
```

**Windows (PowerShell)**

```bash
curl -Method PUT http://hdiprpi:5000/api/leds -Headers @{"Content-Type"="application/json"} -Body '{"state":"on"}'
```

+ Turn it off using ``{"state":"off"}``

```bash
curl -Method PUT http://hdiprpi:5000/api/leds -Headers @{"Content-Type"="application/json"} -Body '{"state":"off"}'
```
