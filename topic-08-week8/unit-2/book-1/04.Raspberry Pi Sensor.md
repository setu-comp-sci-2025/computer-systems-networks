...
In this part, you will extend the week 7 lab that investigated communication for a prototype *Smart Environment* communication scenario. Using the Raspberry Pi, you will implement a prototype sensor that sends real data to the server processes. 
...
+ When  you run the file directly (e.g., you run `python env_data.py`on the command line), the `__name__` variable is set to `"__main__"`, and any code inside the `if __name__ == "__main__":` block will execute. 
...
~~~
$ ping 172.236.14.17
PING 172.236.14.17 (172.236.14.17) 56(84) bytes of data.
64 bytes from 172.236.14.17: icmp_seq=1 ttl=48 time=21.6 ms
64 bytes from 172.236.14.17: icmp_seq=2 ttl=48 time=20.9 ms
64 bytes from 172.236.14.17: icmp_seq=3 ttl=48 time=20.7 ms
~~~
...
serverAddressPort = ("172.236.14.17",41235)
