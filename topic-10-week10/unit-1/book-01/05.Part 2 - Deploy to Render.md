...
In the previous release, its all going on in the RPi, and you cannot access the web dashboard if you are external to your home LAN. The iimage and status is stored on the RPi. We would like to see the dashboard acccessable on the internet 

In this step you’ll perform 2 steps to make this possible. 
...
cloudinary.config(
    cloud_name="YOUR_CLOUD_NAME",   # e.g. " dhnsqvfn8p"
...
You will now use the previous program in your doorbell app. Add the following import to the top of the doorbell_camera.py.
...
Now  update the `<img ..>`  element in templates/status.html one line as indicated in the comments (or just replace the contents of  **`/templates/template.html`** with all the following code):
...
<body>
  ...
  {% if last_event %}
    <p class="meta">
     ...
     <img src="{{ last_event.image  }}">  <!-- CHANGE THIS LINE TO USE THE URL INSTEAD OF LOCAL PATH -->
  {% else %}
...
You can now test the app as before. You will see the same result. This time, the image is stored in Cloudinary and you shoudl see outout like this:

~~~bash
Capturing visitor photo...
Photo saved to: /home/frank/week10-lab1/static/last_visitor.jpg
State saved: {'celcius': 27.76, 'fahrenheit': 81.97, 'image': 'https://res.cloudinary.com/dhnqvdn8p/image/upload/v1764037069/smart-doorbell-lab/last_visitor.jpg', 'ts': 1764037070, 'iso': '2025-11-24T12:17:50'}
~~~

Make sure this works before commiting and pushing you changes to github in the next step. 

Now commit and push your project:

~~~bash
git add .
git commit -m "Images stored on CLoudinary"
git push
~~~
...
Then find you repo in the list and save:
...
If you do not see any images in the browset but just the title, make sure you havr
