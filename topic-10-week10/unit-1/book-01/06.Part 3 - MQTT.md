...
Add the following code Near the top of `doorbell_camera.py` . Replace <YOUR-ID> with your own ID:
...
add the following after `save_state(url)` (towards the end of the script:
...
Run ``python dorrbell_camera.py``. Now each press:

- Captures an image  and stores it in CLoudinary
- Updates the JSON state  with the URL
- Publishes an MQTT message that other services/dashboards can subscribe to  

You can see the entire script in the apprndix of the lab to check it.

#### Add MQTT to web_dashboard.py script

Now we would like the dashboard script to subscribe to the doorbell event and update the state.

Completely replace the `web_dashboard.py` script with the following. Replace <YOUR-ID> with your own ID. **MAKE SURE IT MATCHES WHAT YOU USED IN THE DOORBELL SCRIPT:**
...
MQTT_BROKER = "broker.hivemq.com"
MQTT_PORT = 1883
MQTT_TOPIC = f"/fxwalsh/event/doorbell"  # UPDATE THIS TO YOUR ID
...
On Render, your commit should be picked up automatically and the app will be redeployed.  
...
3. In a browser, go to your render App as before
